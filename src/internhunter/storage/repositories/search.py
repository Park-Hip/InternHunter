...
class SearchRepository:
    ...
    def search_jobs_by_similarity(
        ...
        """
        # TODO: verify whether match_score should stay as a placeholder or be derived from the database distance.
        try:
            with SessionLocal() as session:
                statement = select(CleanJobDB).order_by(CleanJobDB.embedding.op("<=>")(embedding)).limit(limit)
                result = session.execute(statement).scalars().all()
