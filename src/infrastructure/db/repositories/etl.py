...
from src.infrastructure.db.models import Base, RawJobDB, CleanJobDB, AuditJobDB, PipelineRunDB
        ...
        with SessionLocal() as session:
            for table in tables:
                try:
                    ...
                    if not seq_name:
                        continue
                    reset_query = text(f"SELECT setval('{seq_name}', COALESCE((SELECT MAX(id) FROM {table}), 0) + 1, false)")
